...
public class StageManager : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        lootManager.stageManager = this;    // LootManager의 stageManager 할당
    }
    ...
    */
    ...
    // 전투 보상 설정
    public void SetLoot((int golds, bool skill)lootInfoTuple)
    {
        // 전리품UI On
        lootManager.gameObject.SetActive(true);

        // 전리품_골드 획득
        if (lootInfoTuple.Item1 > 0) {
            lootManager.SetLootUI((LootManager.LootType.gold, lootInfoTuple.Item1));
        }

        // 전리품_스킬 획득
        if (lootInfoTuple.Item2) {
            lootManager.SetLootUI((LootManager.LootType.skill, 0));     // skill전리품의 LootInfo 값이 정해지지 않음.
        }
